...
class HealthDataDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final List<HealthData> dataList = controller.healthDataList
        .where((d) => d.tipo == dataType.name)
        .toList()
      ..sort((a, b) => b.dataRegistroDateTime.compareTo(a.dataRegistroDateTime));

    return Scaffold(
      backgroundColor: scaffoldBackgroundColor,
      appBar: AppBar(
        title: Text(dataType.label, style: TextStyle(color: textColor, fontWeight: FontWeight.w600)),
        ...
        children: [
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                Text('Histórico de Registros', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: textColor)),
                Text('${dataList.length} registros', style: TextStyle(fontSize: 14, color: textColor.withOpacity(0.6))),
              ],
            ),
          ),
          Expanded(
  ...
  Widget _buildLineChart(List<HealthData> data) {
    ...
    if (chartData.length < 2) {
      return Center(child: Text('Dados insuficientes para gerar gráfico', style: TextStyle(color: textColor.withOpacity(0.5))));
    }

    List<LineChartBarData> lineBarsData = [];
    final spots = chartData.asMap().entries.map((entry) {
        final value = entry.value.tipo == HealthDataType.pressaoArterial.name ? entry.value.valorSistolica : entry.value.valor;
          ...
          touchTooltipData: LineTouchTooltipData(
            getTooltipItems: (touchedSpots) {
              return touchedSpots.map((spot) {
                ...
                final date = DateFormat('dd/MM/yy').format(dataPoint.dataRegistroDateTime);
  ...
  Widget leftTitleWidgets(double value, TitleMeta meta) {
    return Text(value.toInt().toString(), style: TextStyle(color: textColor.withOpacity(0.6), fontSize: 12), textAlign: TextAlign.left);
  }
  ...
  Widget _buildHealthDataItem(HealthData data, BuildContext context) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(
          data.tipo == HealthDataType.pressaoArterial.name ? data.pressaoArterialFormatada : data.valorFormatado,
